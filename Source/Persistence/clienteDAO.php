<?php
  ...
  class clienteDAO{
    ...
    public function cadastrarCliente($Cliente, $link){
        ...
        $query = sprintf($query, $Cliente->getEmail(), $Cliente->getCpf(),$Cliente->getSexo(),$Cliente->getSenha(),
        $Cliente->getEndereco(),$Cliente->getNome(),$Cliente->getTelefone(),0);
    ...
    public function alterarDados($Cliente, $link){
        ...
        $query = sprintf($query, $Cliente->getEmail(), $Cliente->getCpf(),$Cliente->getSexo(),$Cliente->getSenha(),
        $Cliente->getEndereco(),$Cliente->getNome(),$Cliente->getTelefone(), $Cliente->getEmail());
        echo $query;
        if(!mysqli_query($link,$query)){
